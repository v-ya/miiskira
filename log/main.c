...
const char* miiskira_log_initial(uintptr_t argc, const char *const argv[])
{
	...
	if (
		...
		(miiskira$log$error = inner_miiskira_log_create_bypass(inst, name_error)) &&
		(miiskira$log$warning = inner_miiskira_log_create_bypass(inst, name_warning)) &&
		(miiskira$log$info = inner_miiskira_log_create_bypass(inst, name_info)) &&
		(miiskira$log$verbose = inner_miiskira_log_create_bypass(inst, name_verbose))
	) return NULL;
